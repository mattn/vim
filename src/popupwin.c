...
/*
 * Return TRUE if cell (row, col) is covered by any opacity popup,
 * regardless of zindex comparison.
 */
    int
popup_is_in_any_opacity(int row, int col)
{
    if (opacity_zindex == NULL
	    || row < 0 || row >= opacity_zindex_rows
	    || col < 0 || col >= opacity_zindex_cols)
	return FALSE;
    return opacity_zindex[row * opacity_zindex_cols + col] > 0;
}
